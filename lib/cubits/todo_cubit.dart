...
class TodoCubit extends Cubit<List<Todo>> {
  ...
  Future<void> loadTodos() async {
    final prefs =
        await SharedPreferences.getInstance(); //--->what we declare here? what is the prefs??

    final List<String>? todoJsonList = prefs.getStringList(
      'todos',
    ); //-->prefs.getStringList('todos');--what is todos where it can be anything?
    ...
    final loadedTodos = todoJsonList.map((todoJson) {
      ...
      final Map<String, dynamic> todoMap = jsonDecode(
        todoJson,
      ); //--->make me understand this
  ...
  Future<void> toggleTodo(int index, bool? value) async {
    //--->meake me understand this,i forget this again again
    final updatedTodos = [...state];
